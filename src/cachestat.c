...
const char argp_program_doc[] =
    ...
    "\n"
    "EXAMPLES:\n"
    "    cachestat          # shows hits and misses to the file system page "
    "cache\n"
...
int main(int argc, char** argv) {
  ...
  if (fentry_can_attach("folio_account_dirtied", NULL)) {
    err = bpf_program__set_attach_target(obj->progs.fentry_account_page_dirtied,
                                         0, "folio_account_dirtied");
    if (err) {
  ...
  if (kprobe_exists("folio_account_dirtied")) {
    ...
    bpf_program__set_autoload(obj->progs.tracepoint__writeback_dirty_folio,
                              false);
    bpf_program__set_autoload(obj->progs.tracepoint__writeback_dirty_page,
                              false);
  ...
  while (1) {
    ...
    printf("%8lld %8lld %8llu %7.2f%% %12llu %10llu\n", hits, misses, mbd,
           100 * ratio, buffers / 1024, cached / 1024);
