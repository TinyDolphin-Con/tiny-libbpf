...
#define STARTS_WITH(mapname, prefix) \
  ...
  return mapname[0] &&
         !(STARTS_WITH(mapname, "//anon") ||
           ...
           STARTS_WITH(mapname, "[stack") || STARTS_WITH(mapname, "/SYSV") ||
           STARTS_WITH(mapname, "[heap]") ||
           STARTS_WITH(mapname, "[uprobes]") ||
           STARTS_WITH(mapname, "[vsyscall]"));
}

static bool is_perf_map(const char* path) { return false; }

static bool is_vdso(const char* path) { return !strcmp(path, "[vdso]"); }

static bool is_uprobes(const char* path) { return !strcmp(path, "[uprobes]"); }
...
static int syms__add_dso(struct syms* syms, struct map* map, const char* name) {
  ...
  if (!dso) {
    tmp = realloc(syms->dsos, (syms->dso_sz + 1) * sizeof(*syms->dsos));
    if (!tmp) {
      return -1;
    }
    syms->dsos = tmp;
...
static int dso__load_sym_table_from_perf_map(struct dso* dso) { return -1; }
...
static int dso__load_sym_table_from_elf(struct dso* dso, int fd) {
  ...
  while ((section = elf_nextscn(e, section)) != 0) {
    ...
    if (dso__add_syms(dso, e, section, header.sh_link, header.sh_entsize)) {
      goto err_out;
    }
  }
  ...
  for (i = 0; i < dso->syms_sz; i++) {
    dso->syms[i].name =
        btf__name_by_offset(dso->btf, (unsigned long)dso->syms[i].name);
  }
...
static int create_tmp_vdso_image(struct dso* dso) {
  ...
  while (true) {
    ret = fscanf(f, "%llx-%llx %*s %*x %*x:%*x %*u%[^\n]",
                 (long long*)&start_addr, (long long*)&end_addr, buf);
    if (ret == EOF && feof(f)) {
      break;
    }
    if (ret != 3) {
  ...
  snprintf(tmpfile, sizeof(tmpfile), "/tmp/libbpf_%ld_vdso_image_XXXXXX", pid);
...
struct syms* syms__load_file(const char* fname) {
  ...
  while (true) {
    ret = fscanf(f, "%llx-%llx %4s %llx %llx:%llx %llu%[^\n]",
                 (long long*)&map.start_addr, (long long*)&map.end_addr, perm,
                 (long long*)&map.file_off, (long long*)&map.dev_major,
                 (long long*)&map.dev_minor, (long long*)&map.inode, buf);
    if (ret == EOF && feof(f)) {
      break;
    }
...
struct partitions* partitions__load(void) {
  ...
  while (fgets(buf, sizeof(buf), f) != NULL) {
    ...
    if (sscanf(buf, "%u %u %llu %s", &devmaj, &devmin, &nop, part_name) != 4) {
      goto err_out;
    }
...
bool probe_tp_btf(const char* name) {
  ...
  fd =
      bpf_prog_load(BPF_PROG_TYPE_TRACING, NULL, "GPL", insns, insn_cnt, &opts);
  if (fd >= 0) {
    close(fd);
  }
  return fd >= 0;
